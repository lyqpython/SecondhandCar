...
def admindenglu(request):
    name1 = []
    namelist = admin.adm.all()
    for user in namelist:
        name1.append(user.adminusername)
    glmz=request.POST.get("adminname")
    glmm = request.POST.get("adminpassword")
    list=admin.adm.get(adminusername=glmz)



    adminpassword=list.adminpassword
    if glmz in name1 and adminpassword==glmm:
        return render(request,'guanli.html',{'glmz':glmz})


def useradd(request):
    usernamelist=User.usu.all()
    print(usernamelist)
    namelist=[]
    passwordlist=[]
    tellist=[]
    for use in usernamelist:
        name=use.username
        ...
        tellist.append(tel)
    print(namelist)
    return render(request,"useradmin.html",{"namelist":namelist,"passwordlist":passwordlist,"tellist":tellist})